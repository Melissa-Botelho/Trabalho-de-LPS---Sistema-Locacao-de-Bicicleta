...
public class FrmCadLocacao extends javax.swing.JFrame {
    ...
    public Locacao buscarLocacao (String modelo){
        for (int i =0; i<arrayLocacao.size(); i++){
           if(ControllerLocacao.pegaNomeOuModelo(arrayLocacao.get(i).getIdBicicleta(), 'B').equals(modelo)){
    ...
    public void preencheFormulario(){
       ...
       edtNome.setText(ControllerLocacao.pegaNomeOuModelo(locacao.getIdCliente(), 'C'));
       edtModelo.setText(ControllerLocacao.pegaNomeOuModelo(locacao.getIdBicicleta(),'B'));
       String dataD = locacao.getDataD().toString().replace("-", "");
       ...
       String dataL = locacao.getDataLoc().toString().replace("-", "");
    ...
    public boolean salvarLocacao(){
        try {
            if(tipoEdicao == 'N')    
                locacao = new Locacao();
            locacao.setIdCliente(ControllerLocacao.pegaId(edtNome.getText(),'C'));
            locacao.setIdBicicleta(ControllerLocacao.pegaId(edtModelo.getText(),'B'));
            locacao.setHorarioDev(Float.valueOf(edtHoraD.getText()));
            locacao.setHorarioLoc(Float.valueOf(edtHoraL.getText()));
            locacao.setDataLoc(formatarData.parse(txtDat.getText()));
            locacao.setDataD(formatarData.parse(txtData.getText()));
            locacao.setValorLoc(Float.valueOf(edtVar.getText()));
            if(tipoEdicao == 'N')
                return ControllerLocacao.insert(locacao);
            else if(tipoEdicao == 'E')
                return ControllerLocacao.update(locacao);
    ...
    public Locacao buscar(int id) {
        for (int i = 0; i < arrayLocacao.size(); i++) {
            if (arrayLocacao.get(i).getIdLocacao()==id) {
                return arrayLocacao.get(i);
            }
        }
        return null;
    ...
    public void excluir(){
        if(tblLoc.getSelectedRow()!= -1){
           ...
           preencheFormulario();
            ativaForm(true);
            limparDados();
            if(JOptionPane.showConfirmDialog(rootPane,"Deseja excluir o(a) locacao " + locacao.getIdLocacao(),"Locacao de Bicicletas",JOptionPane.YES_NO_OPTION)== 0){
                ControllerLocacao.delete(locacao.getIdLocacao());
                carregarLocacao();
                ativaForm(false);
    ...
    public final void carregarLocacao(){
        ...
        for(int i=0; i<arrayLocacao.size(); i++){
            Object [] linha = {arrayLocacao.get(i).getIdLocacao(),ControllerLocacao.pegaNomeOuModelo(arrayLocacao.get(i).getIdCliente(),'C') , ControllerLocacao.pegaNomeOuModelo(arrayLocacao.get(i).getIdBicicleta(),'B') , arrayLocacao.get(i).getDataLoc(), arrayLocacao.get(i).getDataD(), arrayLocacao.get(i).getHorarioLoc(),  arrayLocacao.get(i).getHorarioDev(), arrayLocacao.get(i).getValorLoc()};
            modelo.addRow(linha);
        }
    ...
    public boolean verificaDataLocDev(String loc, String dev){
    ...
    public boolean validacaoCampos(){
        ...
        if(edtHoraL.getText().replace(" ", "").matches("[0-9]") || !verificaHora(Float.parseFloat(edtHoraL.getText()))){
            JOptionPane.showMessageDialog(this,"Preencha o horario de locação corretamente (somente numeros)");
            edtHoraL.requestFocus();
            return false;
        }
        if(edtHoraD.getText().replace(" ", "").matches("[0-9]") || !verificaHora(Float.parseFloat(edtHoraD.getText()))){
        ...
        if(edtVar.getText().replace(" ", "").matches("[0-9]")){
         ...
         if(txtDat.getText().replace(" ", "").length() < 10 || !verificaData(txtDat.getText())){
            JOptionPane.showMessageDialog(this,"Preencha a Data de Locação corretamente (deve conter 8 digitos)");
            txtDat.requestFocus();
            return false;
        }
         if(txtData.getText().replace(" ", "").length() < 10 || !verificaData(txtData.getText()) || !verificaDataLocDev(txtDat.getText(),txtData.getText())){
    ...
    public boolean verificaHora(float hora){
